...
export const AdminPortal = ({ user }: { user: User | null }) => {
  ...
  const handleUpdateMenu = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedItem.id) {
        ...
        // Also create a linked inventory item
        await setDoc(doc(db, "inventory", menuRef.id), { 
          id: menuRef.id, 
    ...
    return (
      <div className="h-full flex items-center justify-center bg-primary p-6 lg:p-20">
        <GlassCard className="p-8 lg:p-20 text-center space-y-6 lg:space-y-8 max-w-lg">
          ...
          <p className="text-silver text-xs lg:text-sm leading-relaxed opacity-60">System access restricted to Master Protocol holders.</p>
        </GlassCard>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full bg-primary font-sans text-white overflow-hidden">
        ...
        <div className="space-y-12">
          <div className="px-2">
            <div className="flex justify-between items-center mb-10">
              <p className="text-[9px] uppercase tracking-[0.4em] text-gold font-black opacity-60">Operations Command</p>
            ...
            <nav className="space-y-2">
              {navItems.map((item) => (
                <button 
                  ...
                  className={`w-full flex items-center gap-4 text-[10px] p-4 rounded-2xl transition-all uppercase tracking-[0.2em] font-black ${view === item.id ? 'bg-gold text-black shadow-2xl' : 'text-silver hover:bg-white/5 hover:text-gold'}`}
                >
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-12 xl:p-20 space-y-10 lg:space-y-16 no-scrollbar">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8 pb-8 lg:pb-12 border-b border-white/5">
          <div className="space-y-3">
            <h3 className="text-gold text-[9px] uppercase tracking-[0.6em] font-black opacity-60">System Protocol v8.0</h3>
            <h2 className="text-4xl xl:text-5xl font-serif text-white tracking-tighter uppercase">{view}</h2>
          </div>
          <div className="flex gap-6">
            <div className="xl:text-right">
              <p className="text-[8px] uppercase tracking-widest text-silver mb-1 opacity-60">Global Settlement</p>
            ...
            <div className="xl:text-right border-l border-white/10 pl-6">
              <p className="text-[8px] uppercase tracking-widest text-silver mb-1 opacity-60">Critical Stock</p>
              ...
              {inventory.map((item) => (
                <tr key={item.id} className="group hover:bg-white/[0.02] border-b border-white/[0.02]">
                  <td className="px-6 py-5">
                    ...
                    <p className="text-[8px] text-silver uppercase tracking-[0.2em] mt-2">{item.category}</p>
                  </td>
        ...
        {view === 'menu' && (
          <div className="space-y-10">
            <div className="flex justify-between items-center">
              <SectionTitle subtitle="Gastronomy Configuration" title="Master Menu Registry" />
              <GoldButton onClick={() => { setModalType('menu'); setSelectedItem({ category: 'Bar', price: 0 }); setIsModalOpen(true); }}>
                <PlusIcon className="w-4 h-4 mr-2" /> Add Resource
              </GoldButton>
            </div>
              ...
              {menuItems.map((item) => (
                <GlassCard key={item.id} className="p-8 space-y-6 border-white/5 hover:border-gold/30 transition-all group">
                  <div className="flex justify-between items-start">
                    ...
                    <p className="text-xl font-serif text-white font-black">₦{item.price.toLocaleString()}</p>
                  </div>
                  <div className="space-y-2">
                    <h4 className="text-xl font-bold text-white group-hover:text-gold transition-colors">{item.name}</h4>
                    <p className="text-xs text-silver/60 line-clamp-3 leading-relaxed">{item.description}</p>
                  </div>
                  <div className="flex gap-4 pt-6 border-t border-white/5">
                    ...
                    <button onClick={async () => { if(window.confirm("Purge resource?")) await deleteDoc(doc(db, "menu", item.id)); }} className="p-3 text-red-400/20 hover:text-red-400 hover:bg-red-400/10 rounded-2xl transition-all"><TrashIcon className="w-5 h-5" /></button>
                  </div>
                </GlassCard>
              ))}
            </div>
        ...
        {view === 'staff' && (
          <div className="space-y-10">
            <div className="flex justify-between items-center">
              <SectionTitle subtitle="Personnel Directory" title="Operator Access Points" />
              <GoldButton onClick={() => { setModalType('staff'); setSelectedItem({ role: 'staff_waiter' }); setIsModalOpen(true); }}>
                <UserPlusIcon className="w-4 h-4 mr-2" /> Onboard Personnel
              </GoldButton>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {staff.map((member) => (
                <GlassCard key={member.id} className="p-8 space-y-6 border-white/5 group">
                  <div className="flex justify-between items-start">
                    <div className="w-14 h-14 rounded-2xl bg-black border border-white/10 flex items-center justify-center text-gold font-bold text-2xl shadow-2xl">{member.email?.[0].toUpperCase()}</div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[8px] uppercase tracking-[0.3em] text-silver/40 font-black">Operator Key: {member.id.slice(-8)}</p>
                  </div>
                  <div className="flex gap-4 pt-6 border-t border-white/5">
                    <button onClick={() => { setSelectedItem(member); setModalType('staff'); setIsModalOpen(true); }} className="flex-1 py-3 text-[9px] uppercase font-black tracking-widest text-silver hover:text-gold border border-white/10 rounded-2xl transition-all">Adjust Permissions</button>
                    <button onClick={async () => { if(window.confirm("Revoke access?")) await deleteDoc(doc(db, "admins", member.id)); }} className="p-3 text-red-400/20 hover:text-red-400 rounded-2xl transition-all"><TrashIcon className="w-5 h-5" /></button>
                  </div>
                </GlassCard>
              ))}
            </div>
      ...
      <GlassModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Master Configuration">
        <div className="max-h-[70vh] overflow-y-auto no-scrollbar pr-2">
          {modalType === 'staff' && (
            <form onSubmit={handleUpdateStaff} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-black text-silver">Email Authorization</label>
                <SilverInput icon={EnvelopeIcon} placeholder="Enter operator email" value={selectedItem?.email || ''} onChange={(e: any) => setSelectedItem({ ...selectedItem, email: e.target.value })} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-black text-silver">Department Allocation</label>
                <select value={selectedItem?.role || 'staff_waiter'} onChange={(e) => setSelectedItem({ ...selectedItem, role: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white focus:outline-none focus:border-gold/30">
                  <option value="staff_waiter">Kitchen Terminal (Waiter)</option>
                  <option value="staff_bar">Bar Terminal</option>
                  <option value="admin">Global Protocol (Admin)</option>
                </select>
              </div>
          ...
          {modalType === 'menu' && (
            <form onSubmit={handleUpdateMenu} className="space-y-6">
              <SilverInput placeholder="Item Designation" value={selectedItem?.name || ''} onChange={(e: any) => setSelectedItem({ ...selectedItem, name: e.target.value })} />
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-black text-silver">Category Registry</label>
              ...
              <textarea placeholder="Transmission Description" value={selectedItem?.description || ''} onChange={(e) => setSelectedItem({ ...selectedItem, description: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm text-white h-32 focus:outline-none" />
          ...
          {modalType === 'item' && (
            <form onSubmit={handleUpdateInventory} className="space-y-6">
              <div className="p-6 bg-black/40 rounded-[24px] border border-white/5 text-center space-y-2">
                <p className="text-[10px] uppercase font-black text-gold">Resource Synchronization</p>
                <p className="text-xl font-serif text-white">{selectedItem?.name}</p>
              </div>
              <SilverInput type="number" placeholder="Authorized Stock Level" value={selectedItem?.stock || 0} onChange={(e: any) => setSelectedItem({ ...selectedItem, stock: Number(e.target.value) })} />
              <GoldButton type="submit" className="w-full py-5 text-[11px]">Commit Stock Level</GoldButton>
            </form>
          )}
        </div>
      </GlassModal>
